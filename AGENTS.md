...
- 必要なキー: `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY`, `DATABASE_URL`, `CORPUS_DIR`。`LANGFUSE_*` は**任意**（未設定時は計装が no-op になり、アプリ・デモ・eval は動作する。requirements NFR-4/NFR-8）。増分再取り込み関連の `INGEST_*` 設定は任意（既定値あり。`core/config.py` 参照）。
