...
├── Makefile
├── docker-compose.yml
├── .devcontainer/              # VS Code Dev Container設定（任意。backendのみ対象。既存apiサービスに
│                               #   アタッチするだけ。docs/specs/26072417-backend_devcontainer/spec.md）
├── docs/
│   ├── requirements.md
│   ├── architecture.md
│   ├── db_design.md
...
- 必要なキー: `OPENAI_API_KEY`, `VOYAGE_API_KEY`, `DB_HOST`/`DB_PORT`/`DB_USER`/`DB_PASS`/`DB_NAME`（まとめてDB接続情報を構成。`DATABASE_URL` を明示指定した場合はそちらが優先される。`docker-compose.yml` の `api`/`ingest_worker` サービスは `DB_HOST=db` のみを上書きする）, `CORPUS_DIR`。`LANGFUSE_*` は**任意**（未設定時は計装が no-op になり、アプリ・デモ・eval は動作する。requirements NFR-4/NFR-8）。増分再取り込み関連の `INGEST_*` 設定は任意（既定値あり。`core/config.py` 参照）。
...
| `make api` | API 起動（`docker compose up --build --no-deps api`。`Dockerfile.local`のCMDで`uvicorn --reload`が動く） |
| `make web` | フロント起動（`pnpm --dir frontend dev`） |
| `make worker` | Google Drive取り込みジョブ（API経由トリガ）を処理するARQ workerをコンテナで起動 |
| `make ingest CORPUS=path/` | コーパス取り込み（CLI。`docker compose run --rm --build api`経由で実行） |
| `make demo` | シードコーパスで即デモ可能な状態にする（コンテナ経由） |
| `make ingest-gdrive` | Google Driveフォルダを取り込む（CLI。コンテナ経由） |
| `make test` | `pytest`をコンテナ経由で実行。`DB_NAME=rag_test`に上書きして`rag_dev`を保護する |
| `make lint` | backend: ruff/mypyをコンテナ経由（`--no-deps`）で実行 / frontend: `biome lint .` + `biome format .`（チェックのみ） |
| `make fmt` | backend: `ruff format .`をコンテナ経由（`--no-deps`）で実行 / frontend: `biome format --write .` |
| `make eval` | Evalハーネスをコンテナ経由で実行（既存e2e eval + M7複合質問の補足書式検証） |
| `make eval-routing` | M7 routing evalをコンテナ経由で実行（rewrite→retrieve→gradeを評価。generateは実行しない） |
| `make eval-all` | `make eval` + `make eval-routing` |
| `make migrate` | Alembicマイグレーションをコンテナ経由で実行 |
| `make openapi` | OpenAPIスキーマを出力（DB/Redis非依存のためネイティブの`uv run`のまま） |

> 個別に `pip install` / `python xxx.py` を叩かず、原則 Make ターゲットを使うこと（各ターゲットは
> `docker compose run --rm --build api ...` 経由でコンテナ内の `uv run`/`pytest`/`ruff`/`mypy` 等を実行する。
> `make setup` のホスト側 `uv sync`（IDE用 `.venv` 構築）と `make openapi`（DB非依存のコード生成）のみ例外）。
...
- ❌ シークレット（API キー）・個人の実データをコミットする
- ❌ プロンプトをコードにハードコードする（`prompts/` を使う）
- ❌ Eval を回さずにプロンプト / チャンキング / 埋め込みを変更する
