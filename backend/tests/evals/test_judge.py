...
@patch("private_rag_apps.evals.judge._call_judge")
def test_evaluate_direct_groundedness(mock_call_judge):
    """M7 T4: direct経路の回答がコーパス固有の内容を捏造していないかのjudge
    （スペック rev.3 §7.2 direct groundedness）"""
    mock_call_judge.return_value = {"score": 1, "rationale": "ok"}
    result = evaluate_direct_groundedness("Pythonのwalrus operatorとは?", "一般的な回答です。")
    assert result["score"] == 1
    assert mock_call_judge.called


@patch("private_rag_apps.evals.judge._call_judge")
def test_evaluate_supplement_format(mock_call_judge):
    """M7 T4: 複合質問への回答が「一般知識に基づく補足」書式を守れているかのjudge
    （スペック rev.3 §7.2 補足書式の遵守）"""
    mock_call_judge.return_value = {"score": 1, "rationale": "ok"}
    result = evaluate_supplement_format(
        "複合質問", "本文[1]\n\n---\n## 一般知識に基づく補足\n一般論"
    )
    assert result["score"] == 1
    assert mock_call_judge.called
