...
rewrite ノード自体（rewrite_applied の追跡・temperature=0固定・グラフ内ノード化）は
T5 のスコープであり、本スクリプトはそれを実装しない。ただし、rewrite に使う想定の
`generation.condense(query, history)` 関数自体は M2/M3 で実装済みであり、現在の
`/api/chat` ハンドラも履歴がある場合はグラフの外からこれを直接呼び出している
（AGENTS.md §3 の「暫定例外」）。本スクリプトはこの実際の挙動を忠実に再現するため、
`history` が非空の項目（followupカテゴリ）については実際に `condense()` を呼び出す。
history が空の項目（corpus/general/ambiguous）は search_query = query のまま
（この場合 condense() を呼んでも query がそのまま返るだけで実質的に同じ）。

condense() 自体に temperature=0 の指定は無い（T5 で追加予定。既知の制約）ため、
followup項目の rewrite 結果は非決定的でありうる。condense() 失敗時は自動的に
元のqueryにフォールバックする（condense()自身の既存フォールバック）ため、
LLMが一切使えない環境でも本スクリプトはpass-through相当の動作で完走できる。

`--cached-rewrite` はこの rewrite 結果(search_query)を jsonl にキャッシュし、
再実行時（閾値変更後の再集計など）に condense() の再呼び出しを避けるためのオプション。
...
def resolve_search_query(
    ...
    """rewrite段階: historyが非空ならcondense()を呼ぶ(実際の/api/chatと同じ挙動。
    モジュールdocstring参照)。--cached-rewrite指定時はキャッシュがあればそれを使う。
    戻り値: (search_query, rewrite_applied)"""
    ...
    if use_cache and item["id"] in rewrite_cache:
        ...
        return cached["search_query"], cached["rewrite_applied"]

    search_query = condense(item["query"], history)
    rewrite_applied = search_query.strip() != item["query"].strip()
...
def process_dataset(args: argparse.Namespace) -> None:
    ...
    try:
        for i, item in enumerate(dataset):
            ...
            try:
                search_query, rewrite_applied = resolve_search_query(
