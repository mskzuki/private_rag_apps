...
class Settings(BaseSettings):
    ...
    # Google Drive Ingestion Settings (M9)
    drive_folder_id: str = ""  # 取り込み対象のDriveフォルダID（空文字列 = Drive機能無効。フォルダURL末尾のID部分）
    ...
    ingest_gdrive_job_timeout_sec: int = 3600  # ARQジョブのタイムアウト秒（既定3600=1時間。ARQ既定の300秒だとINGEST_EMBED_MIN_INTERVAL_SEC等のペーシングを含む実際の取り込みには短すぎるため延長）
