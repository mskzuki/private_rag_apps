...
- Status: Accepted (rev.4 — T7 で M7 実装完了、実装との差分・ADR 参照を反映)
...
- Blocked by: なし
- Blocks: M8 候補 (clarify / HITL / LLM grader。ADR 0005 により別スペックとして起票予定)
...
**注（T7 追記、rev.4）: 本表は THETA・grade ロジックの将来的なチューニング目標として残すが、M7 実装完了時点でこの基準を完全には満たしていない。** 実績と根拠 ADR は以下の通り（詳細: `docs/specs/m7_tasklist.md` T4/T7、`docs/adr/0001_m7_theta_threshold.md`、`docs/adr/0005_m7_no_go_accepted.md`）:

| 指標 | 実績（holdout） | 基準を満たすか |
|---|---|---|
| grounded 見逃し | 0/21 | 達成（≤ 1 件） |
| direct 誤り | 4/18 | **NO-GO**（基準 ≤ 3 件を超過。ADR 0005 によりユーザーが受理済み。原因は Voyage rerank 一時失敗時の安全側フォールバック（迷ったら grounded）と一致することが判明したが、再取得は行わず判定をそのまま採用） |
...
閾値方式で grounded 見逃し・direct 誤りを holdout 上で同時に満たせないことが示されたため、§7.4 の exit criteria を満たしたと判断し、グレーゾーン限定の LLM grader を別スペックとして起票する方針が ADR 0005 で承認済み（本スペックのスコープ外、別途起票）。
