...
- Status: Draft (rev.3 — 実装前レビューで判明した現行コードとの差分を反映)
...
詳細は `M7-tasks.md`。実装順序: **T0 → T1 → T2 →（GO/NO-GO）→ T3 → T4 → T5 → T6 → T7**
...
## 10. 未決事項（実装中に決定してよいもの）

- rewrite に渡す履歴ターン数 N の最終値（初期値 6、Langfuse で入力トークンとレイテンシを観測して調整）
- 「一般知識に基づく補足」セクションの文言・書式の最終形（見出し文字列は本スペックの表記を初期値とする）
- LLM-as-judge（direct groundedness / 補足書式）に使うモデルとプロンプト