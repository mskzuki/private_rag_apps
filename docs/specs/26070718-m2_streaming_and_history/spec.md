# Private RAG Apps — M2 フィーチャースペック: ストリーミング + 出典 UI + 会話履歴 (docs/specs/26070718-m2_streaming_and_history/spec.md)

> 配置先: `docs/specs/26070718-m2_streaming_and_history/spec.md`
...
#### SSE イベント仕様への M7 追加分（後方互換）

M7（`docs/specs/26071422-m7_adaptive_routing/spec.md` rev.3 §5.2）で LangGraph 導入に伴い、グラフ実行の透明性向上のため以下を追加した。**上記 M2 確定版の 4 イベント（`citations`/`token`/`done`/`error`）のペイロードは無変更**であり、以下は追加のみ（既存クライアントは未知イベント型を無視する実装のため後方互換。`backend/tests/test_chat_sse_structure.py` の構造検証テストで担保）。

| event | data | 送出タイミング |
|---|---|---|
| `node_start` | `{"node": "rewrite" \| "retrieve" \| "grade" \| "generate"}` | 各グラフノード開始時 |
| `route_decided` | `{"route": "grounded" \| "direct", "kept": int, "dropped": int, "top_score": float \| null}` | grade ノード完了時（`top_score` は retrieved 先頭チャンクの rerank_score。retrieved が空なら `null`） |
| `rewrite_result` | `{"applied": bool, "query": str}` | rewrite ノード完了時（UI でのデバッグ表示用） |

フロントエンド（`frontend/src/lib/chat-adapter.ts`）は `route_decided` のみ `metadata.custom.route` に反映してバッジ表示（`RouteBadge.tsx`）に使い、`node_start`/`rewrite_result` は受信のみで表示しない（M7 では最小実装。リッチな進捗 UI は M5 showcase の磨き込みの範疇）。
...
- [x] 生成失敗時は `error` イベントが送られ、**そのターンが user / assistant とも保存されない**（履歴の交互列が保たれる）— `api/main.py:192-201` で `has_error=True` を設定、L203 `if not has_error:` が Bulk Save をガード（DB保存自体をスキップ）。※専用の統合テスト（エラー発生ターンの非保存を直接検証するもの）は未整備（`docs/specs/26070718-m2_streaming_and_history/tasklist.md` Phase 2 参照）
- [x] クライアント切断で LLM ストリームが中断され、部分結果が保存されない — `api/main.py:171-173` `await request.is_disconnected()` でループを `break`、`has_error=True` として Bulk Save をスキップ。※OpenAI ストリーム自体の明示的な `close()`/キャンセル呼び出しはなく CPython の GC 経由の暗黙的な中断に依存しており、専用テストも無い（動作の弱点として `docs/specs/26070718-m2_streaming_and_history/tasklist.md` に注記）
...
- [x] 検索 0 件で「該当する情報が見つかりませんでした」を返し、でっち上げない — `generation/generator.py:42-45`、`tests/test_chat.py::test_generate_answer_stream_no_chunks` で検証。※§4.6 が既定とする「保存しない」は未実装（実装は常に Bulk Save する。本項目文言＝でっち上げ防止自体は満たすが、乖離として `docs/specs/26070718-m2_streaming_and_history/tasklist.md` に注記）
...
AGENTS.md §12 に従い、**本スペック（`docs/specs/26070718-m2_streaming_and_history/spec.md`）→ タスクリスト（`docs/specs/26070718-m2_streaming_and_history/tasklist.md`）→ 実装**の順で進める。実装は概ね次の依存順:
...
> 次に作成すべき成果物は **`docs/specs/26070718-m2_streaming_and_history/tasklist.md`**（チェックボックスでの進捗管理。AGENTS.md §12）。
...
| version | 日付 | 変更 |
|---|---|---|
| v0.3 | 2026-07-16 | M7 T6 追従: §4.1 に SSE イベント仕様への M7 追加分（`node_start`/`route_decided`/`rewrite_result`。後方互換、既存4イベントのペイロード無変更）を追記。詳細は `docs/specs/26071422-m7_adaptive_routing/spec.md` rev.3 §5.2 |
