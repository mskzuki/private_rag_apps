...
├─ retrieve
│  ├─ vector_search (SQL latency)
│  ├─ fts_search    (SQL latency)
│  ├─ rrf_fuse      (件数)
...
- **pg_bigm の類似度しきい値**: `=%` 演算子は `pg_bigm.similarity_limit`（既定0.3）で足切りする。短いクエリで再現率が落ちる場合、しきい値を下げるか、`=%` フィルタを外して `bigm_similarity` 順で `LIMIT` するかを Eval で決める（M1-2 で計測）。
- **rerank 投入数（FUSE_K=40）のコスト/精度トレード**: rerank はトークン課金。Eval で 40 を増減して nDCG@10 とコストのバランスを見る。
- **候補数 CANDIDATE_K=50 の妥当性**: Recall@10 が頭打ちなら削ってレイテンシを稼ぐ余地。
