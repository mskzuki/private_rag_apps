...
M7（`docs/specs/m7_adaptive_routing.md` rev.3 §5.2）で LangGraph 導入に伴い、グラフ実行の透明性向上のため以下を追加した。**上記 M2 確定版の 4 イベント（`citations`/`token`/`done`/`error`）のペイロードは無変更**であり、以下は追加のみ（既存クライアントは未知イベント型を無視する実装のため後方互換。`backend/tests/test_chat_sse_structure.py` の構造検証テストで担保）。
...
| version | 日付 | 変更 |
|---|---|---|
| v0.3 | 2026-07-16 | M7 T6 追従: §4.1 に SSE イベント仕様への M7 追加分（`node_start`/`route_decided`/`rewrite_result`。後方互換、既存4イベントのペイロード無変更）を追記。詳細は `docs/specs/m7_adaptive_routing.md` rev.3 §5.2 |
