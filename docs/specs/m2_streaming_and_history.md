...
- [ ] `ttft_ms` と検索レイテンシが記録され、p95 ベースラインがドキュメント化されている — `ttft_ms` の記録自体は実装済み（`api/main.py:180-184` `get_client().update_current_trace(metadata={"ttft_ms": ...})`）が、**p95 ベースラインのドキュメント化は未実施**。`requirements.md`/`docs/specs/m2_streaming_and_history.md` §7 とも「実測後に確定」という将来形の記述のみで実測値が無く、`m5_showcase_finishing.md` でも Langfuse トレースのスクショ取得は「未実行」の扱い（Docker/実行環境が必要。M5 Phase 3 待ち）
