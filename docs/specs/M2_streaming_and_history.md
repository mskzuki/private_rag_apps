...
sequenceDiagram
    ...
    API->>DB: assistant メッセージ + citations を保存
    API-->>U: SSE: done {message_id, conversation_id}
```

> **エラー時**: 生成中の失敗は `SSE: error` を送出し、assistant メッセージは保存しない（§4.6）。user メッセージは保存済みのため、再読み込み時は「未応答の質問」として残る（クライアントは retry 導線を出す）。
...
| タイミング | 操作 | 備考 |
|---|---|---|
| リクエスト受信直後 | `messages`(role='user') を INSERT | 失敗ターンでも質問が履歴に残る |
| `done` 直前 | `messages`(role='assistant', content, citations) を INSERT | **正常終了時のみ**。`message_id` を採番して `done` で返す |
| いずれの保存後 | `conversations.updated_at` を更新 | 一覧のソート用 |

- **部分保存はしない**: エラー時に assistant メッセージを保存しない（ハルシネーションの断片を履歴化しないため。NFR-6）。
- `messages.citations`（jsonb）には §4.5 の citations 配列をそのまま格納（db_design §4 の既存カラムを使用。**DDL 変更なし**）。
- **スキーマ変更は不要**: M2 は `conversations` / `messages`（0001_init で作成済み）を利用するのみ。マイグレーションは追加しない（db_design §4/§8 と整合）。
...
- 回答本文中の `[n]` は対応する出典カードへスクロール/ハイライトするアンカーにする。
...
- **目標値**: requirements §NFR-2 のとおり「実測後に確定」。M2 では Langfuse の `ttft_ms` / 検索レイテンシで **p95 のベースラインを取得**し、目標（例: TTFT p95 < 2.0s、検索 p95 < 500ms）を暫定設定してドキュメント化する。
...
既存の必須キー（`ANTHROPIC_API_KEY` / `VOYAGE_API_KEY` / `LANGFUSE_*` / `DATABASE_URL` / `CORPUS_DIR`）は変更なし。
...
**ストリーミング（FR-4）**
- [ ] `/api/chat` が `text/event-stream` を返し、`citations`→`token`(逐次)→`done` の順で配信される
- [ ] 生成失敗時は `error` イベントが送られ、assistant メッセージが保存されない
- [ ] クライアント切断で LLM ストリームが中断され、部分結果が保存されない

**出典 UI（FR-5, NFR-6）**
- [ ] 回答本文の `[n]` と出典カードが対応し、クリックで title / path / heading を表示できる
- [ ] 本文に出現しない番号の出典カードは最終表示に出ない
- [ ] 検索 0 件で「該当する情報が見つかりませんでした」を返し、でっち上げない

**マルチターン（FR-5）**
- [ ] 「それの詳細は?」等のフォローアップが直前話題を補完して検索される（condense が効く）
- [ ] 初回ターンでは condense を呼ばない

**会話履歴（FR-6）**
- [ ] 会話が保存され、一覧に updated_at 降順で並ぶ
- [ ] 会話を選択して履歴を復元し、続きを送信できる

**assistant-ui / 保守性（NFR-7）**
- [ ] streaming / auto-scroll / retry / スレッド切替を自前実装していない（assistant-ui に委譲）

**可観測性・性能（NFR-4, NFR-2）**
- [ ] 1 チャット = 1 トレースで condense / generate(stream) が span 化され、トークン/コストが記録される
- [ ] `ttft_ms` と検索レイテンシが記録され、p95 ベースラインがドキュメント化されている

**共通（AGENTS.md §10）**
- [ ] `make lint` / `make test` が通る（新規ロジックにテストあり）
- [ ] 依存方向ルール（§3）を破っていない
- [ ] 本スペックが §12 の変更を `requirements.md` / `architecture.md` に反映済み

---

## 11. テスト方針（AGENTS.md §8）

- **ユニット**: SSE イベント整形、`[n]` フィルタ（出現番号抽出）、condense のスキップ判定、タイトル切り詰め、履歴のトークン切り詰め。
...
1. バックエンド: `/api/chat` の SSE 化（citations→token→done→error）＋キャンセル
2. バックエンド: 会話の永続化 + `POST/GET /api/conversations`・`GET /api/conversations/{id}`
3. バックエンド: condense（初回スキップ・プロンプト外だし・トレース）
4. フロント: `useLocalRuntime`(ChatModelAdapter) で SSE 受信・累積 yield
5. フロント: 出典カード（generative UI・`[n]` フィルタ・クリック表示）
6. フロント: `useRemoteThreadListRuntime` + 履歴復元
7. 可観測性: `ttft_ms`・condense span・cancelled 処理／p95 ベースライン記録
