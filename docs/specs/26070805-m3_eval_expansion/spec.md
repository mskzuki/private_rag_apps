...
- Eval レポートの公開（人間可読サマリの自動生成。`docs/eval_report.md` は M5 がこのサマリを引用して構成する）
...
- [x] before/after が PR に自動記載される — `.github/workflows/eval.yml:71-86`（`backend/evals/reports/latest_summary.md` が存在すれば `github-script` で PR コメントとして投稿。M5 監査で `docs/eval_report.md` への直接書き込みから分離。§5.1/§5.3）
